...
def objective_psll(I0):
    ...
    for num in range(Ns):
        E0[num] = np.sum(np.exp(1j * 2 * np.pi * I0) * np.exp(1j * (Wavenumber * X * np.sin(np.deg2rad(theta[num])) + alpha)))
    ...
    SLL_start = np.where(AF_inc)[0][0] if np.any(AF_inc) else len(E0_dB) - MPeak  # find the first right null
    SLL_r = np.max(E0_dB[MPeak + SLL_start-1:])

    AF_inc = np.diff(E0_dB[:MPeak]) < 0  # precompute the pattern diff
    SLL_start = np.where(AF_inc)[0][-1] if np.any(AF_inc) else 0  # find the first left null
    SLL_l = np.max(E0_dB[:SLL_start+1])

    SLL = np.max([SLL_r, SLL_l]) # Select the maximum SLL
    return SLL
...
def on_generation(ga_instance):
    ...
    if ga_instance.generations_completed % maxGeneValue == 0:  # Plot end of the generation of the original pattern       
        PlotPattern(len(best_solution), np.zeros(len(best_solution)))
...
ga_instance = pygad.GA(
    ...
    parallel_processing = 10, # Number of parallel processes to use, default is 1.
